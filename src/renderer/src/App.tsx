...
export default function App(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const offLoop = window.topolome.onLoopStatus(setLoop);
    // Refresh the sidebar when the agent adds/removes category folders.
    const offStore = window.topolome.onStoreChanged(refresh);
    return () => {
      offLoop();
      offStore();
    };
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden">
      ...
      >
        <AppLogo />
        <span className="ml-2 text-xs font-semibold tracking-widest uppercase">topolome</span>
        <Button
          variant="ghost"
          size="icon-sm"
      ...
      <div className="flex flex-1 overflow-hidden">
        <aside className="flex w-64 shrink-0 flex-col border-r border-border bg-card">
          <div className="flex-1 overflow-y-auto py-2">
            <CategoryList categories={categories} onRename={handleRename} onDelete={handleDelete} />
          </div>
          ...
          <div className="border-t border-border p-3">
            ...
            <p className="mt-3 truncate text-[11px] text-muted-foreground" title={root}>
              {root || "~/.topolome"}
            </p>
          </div>
        </aside>
